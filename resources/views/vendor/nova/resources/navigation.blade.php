@if (count(\Laravel\Nova\Nova::resourcesForNavigation(request())))
    ...
    @foreach($navigation as $group => $resources)
        @if (count($groups) > 1)
            <h4 class="ml-8 mb-4 text-xs text-white-50% uppercase tracking-wide"><b>{{ $group }}</b></h4>
        @endif
            ...
            @foreach($resources as $resource)
                <li class="leading-tight mb-4 ml-8 text-sm">
                    <router-link :to="{
                        ...
                        @if (!is_null($notification) && count($notification))
                            @if (array_key_exists($resource, $notification))
                                <div class="label-div">
                                    {{ $resource::label() }}
                                </div>
